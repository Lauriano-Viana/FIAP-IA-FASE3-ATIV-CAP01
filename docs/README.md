...
2. **Análise dos Dados:**
   * O sistema compara os dados coletados com os valores ideais definidos para cada cultura (armazenados na tabela `culturas`).
...
4. **Registro:**
   * Cada ação de irrigação é registrada na tabela `irrigacoes`, juntamente com a leitura que a motivou.
## Operações CRUD
* **Sensores:**
